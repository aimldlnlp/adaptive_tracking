...
SESSION_NAME="adaptive_tracking_best_run"
SUITE_PATH="configs/experiments/paper_best.yaml"
...
if [[ "$CURRENT_TMUX_SESSION" != "$SESSION_NAME" ]]; then
  ...
  LOG_PATH="$PROJECT_ROOT/outputs/logs/run_suite_${TIMESTAMP}.log"
  STATUS_PATH="$PROJECT_ROOT/outputs/logs/latest_progress.json"

  tmux new-session -d -s "$SESSION_NAME" \
    "cd '$PROJECT_ROOT' && ADAPTIVE_TRACKING_LOG_PATH='$LOG_PATH' ADAPTIVE_TRACKING_STATUS_PATH='$STATUS_PATH' bash -lc './scripts/run_suite_tmux.sh --suite $SUITE_PATH'"
...
LOG_PATH="${ADAPTIVE_TRACKING_LOG_PATH:-$PROJECT_ROOT/outputs/logs/run_suite_$(date +%Y%m%d_%H%M%S).log}"
...
exec "$PROJECT_ROOT/.venv/bin/python" "$PROJECT_ROOT/scripts/run_suite_with_progress.py" \
  --suite "$SUITE_PATH" \
  --log-path "$LOG_PATH" \
